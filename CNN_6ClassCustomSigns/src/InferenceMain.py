...
def load_model(ModelPath):
    ...
    base_model.load_state_dict(torch.load(os.path.join(ModelPath, 'Final_model.pkl'), map_location=lambda storage, loc: storage))
    return base_model
...
def load_dataset(path):
    ...
    for i, video_file in enumerate(video_files):
        ...
        if int(video_number) < len(video_files) - 6:
...
def run_test(model_path,img_path):
    ...
    data_loader_test = get_temporal_loader(test_path_list, 1, shuffle=False,transform=VAL_TRANSFORM, num_workers=4)
    model = load_model(20, model_path)
...
# run_test(args.modelPath,args.imgDir)
print(run_test("/Users/abhinaavramesh/Downloads/sign-language-recognition-master/src/model/early_fusion_sgd/","/Users/abhinaavramesh/Downloads/sign-language-recognition-master/data/ms-asl/rgb/test/0/"))
