...
class BaseModel2(nn.Module):
    ...
    def __init__(self, in_channels, out_classes, flatten_size, fuse_early=0):
        ...
        # BLOCK 5
        layers_conv.append(nn.Conv2d(in_channels=512, out_channels=256,
                                     kernel_size=3, padding=1, stride=1))
        layers_conv.append(nn.ReLU(inplace=True))

        # BLOCK 6
        layers_conv.append(nn.Conv2d(in_channels=256, out_channels=128,
                                     kernel_size=3, padding=1, stride=1))
        layers_conv.append(nn.ReLU(inplace=True))
...
class FinalFcLayer2(nn.Module):
    def __init__(self, out_classes):
        super(FinalFcLayer2, self).__init__()
        self.final_fc = nn.Linear(1024, out_classes)
        self._initialize_weights()
        print(self.final_fc)
